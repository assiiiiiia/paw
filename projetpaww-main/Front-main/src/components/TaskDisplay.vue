...
export default {
  ...
  methods: {
    ...
    const updatedTask = { 
      ...this.task, 
      completed: isChecked,
      status: isChecked ? 'termine' : this.task.status // Update the status based on checkbox state
    };
    this.$emit("update-task", updatedTask); // Emit updated task to the parent component
  },
},
  mounted() {
